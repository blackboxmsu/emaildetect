export interface GeoLocationData {
  ...
  isp?: string;
  organization?: string;
  infraType: 'RESIDENTIAL' | 'CORPORATE' | 'CLOUD_HOSTING' | 'VPN_PROXY' | 'TOR_EXIT' | 'UNKNOWN';
  threatFlags: string[];
  reverseDns?: string;
}
...
const INFRASTRUCTURE_DB: Record<string, Partial<GeoLocationData>> = {
...
export async function lookupGeoIP(ip: string): Promise<GeoLocationData> {
  if (!ip) {
    return {
      ip: '0.0.0.0',
      country: 'Unknown',
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.status === 'success') {
        ...
        let infraType: GeoLocationData['infraType'] = 'RESIDENTIAL';
...
export function analyzeDomainIntel(domain: string): DomainIntelData {
  ...
  let reputation: DomainIntelData['reputation'] = 'SAFE';
