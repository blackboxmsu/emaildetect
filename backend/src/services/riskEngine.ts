import { AuthenticationAnalysisResult } from './authAnalysis.js';
import { HeaderForensicsResult } from './headerForensics.js';
import { NLPThreatAnalysis } from './nlpThreatEngine.js';
import { GeoLocationData, DomainIntelData } from './geoIpIntel.js';
...
export interface RiskScoreResult {
  ...
  breakdown: {
    authenticationWeight: number; // 0-30
    nlpUrgencyAndBecWeight: number; // 0-35
    domainReputationWeight: number; // 0-20
    infrastructureAndIpWeight: number; // 0-15
    attachmentAndUrlWeight: number; // 0-15
  };
  classification: {
    category: 'BENIGN' | 'SUSPICIOUS' | 'PHISHING' | 'BEC_FRAUD' | 'EXECUTIVE_IMPERSONATION' | 'MALWARE_DROPPER';
...
export function calculateRiskAndConfidence(
  ...
  nlpWeight += Math.min(10, nlp.urgencyCues.length * 4);
  nlpWeight = Math.min(35, nlpWeight);

  // 3. Domain Reputation Weight (up to 20)
  if (nlp.lookalikeDomains.length > 0) domainWeight += 16;
  ...
  if (headers.routingAnomalies.length > 0) infraWeight += 6;
  ...
  let fraudLevel: RiskScoreResult['fraudLevel'] = 'LOW';
  ...
  // Section 9: Measures how strongly available technical evidence supports the inferred source infrastructure
  ...
  if (headers.relayPath.length >= 2) attrScore += 10;
  ...
  if (headers.routingAnomalies.length > 1) attrScore -= 15; // Corrupted / forged hops

  const attributionConfidence = Math.max(15, Math.min(95, Math.round(attrScore)));

  // Threat Classification
  let category: RiskScoreResult['classification']['category'] = 'BENIGN';
  ...
  } else if (nlp.credentialHarvesting.detected || (nlp.lookalikeDomains.length > 0 && suspiciousUrlCount > 0)) {
    category = 'PHISHING';
