...
app.post('/api/analyze/sample/:id', async (req: Request, res: Response) => {
  try {
...
// Upload .eml file
app.post('/api/analyze/upload', upload.single('file'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
app.post('/api/analyze/raw', async (req: Request, res: Response) => {
  try {
...
async function persistCase(report: any) {
  const caseDoc = {
    ...
    urls: report.parsedEmail.urls.map((u: string) => ({ url: u, domain: u, suspicious: false })),
